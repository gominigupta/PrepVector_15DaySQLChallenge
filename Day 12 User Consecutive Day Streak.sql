...
-- Step 1: Extract unique visit dates per user for 2020
WITH user_visits AS (
    SELECT
        user_id,
        DATE(created_at) AS visit_date
    FROM events
    WHERE YEAR(created_at) = 2020  -- Keep only events from 2020
    GROUP BY user_id, visit_date              -- Ensure one visit per user per day
),

-- Step 2: Get the previous visit date for each user
visits_with_lag AS (
    SELECT
        user_id,
        visit_date,
...
-- Step 3: Flag the start of a new streak if the gap from previous date is more than 1 day
visit_flags AS (
    SELECT
        user_id,
        visit_date,
        CASE
            WHEN DATEDIFF(visit_date, prev_date) = 1 THEN 0  -- continuation of streak
            ELSE 1                                                        -- start of new streak
        END AS new_streak_flag
    FROM visits_with_lag
),

-- Step 4: Assign a streak group ID to each visit using running total of flags
streak_grouped AS (
    SELECT
        user_id,
        visit_date,
        SUM(new_streak_flag) OVER (PARTITION BY user_id ORDER BY visit_date) AS streak_id
    FROM visit_flags
),

-- Step 5: Count number of days in each streak group
streaks AS (
    SELECT
        user_id,
        streak_id,
...
-- Step 6: Get each user's longest streak and return top 5
SELECT
    user_id,
