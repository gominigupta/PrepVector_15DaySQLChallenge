...
-- Given a table of customer sales in a retail store with columns 
-- id, transaction_value, and created_at representing the date and time for each transaction, 
...
(SELECT 
        ...
        created_at, 
        transaction_value,
        ROW_NUMBER() OVER (PARTITION BY DATE(created_at) ORDER BY created_at DESC) AS txn_rank
    FROM customer_sales)
