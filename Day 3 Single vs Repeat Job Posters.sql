...
SELECT 
    COUNT(CASE WHEN max_post_count = 1 THEN 1 END) AS single_post_users,
    COUNT(CASE WHEN max_post_count > 1 THEN 1 END) AS multiple_post_users
FROM (
    SELECT user_id, MAX(post_count) AS max_post_count
    FROM (
